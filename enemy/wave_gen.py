...
class Wave_gen:
    ...
    def Generate_wave(self, wave_number: int) -> dict[int, tuple[int, data_class.SpecialEnemyTypes]]:
        ...
        if self.data.difficulty in ["operational", "overclocked"]:
            ...
            self.config.config.time_increase_fix[0] -= 16
            self.config.config.time_increase_fix[1] -= 12
            self.config.config.time_increase_fix[2] -= 8
        if self.data.difficulty in ["critical"]:
            self.config.config.base_time = 60*40
            self.config.config.time_increase_fix[0] -= 30
            self.config.config.time_increase_fix[1] -= 24
            self.config.config.time_increase_fix[2] -= 18
